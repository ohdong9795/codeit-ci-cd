...
const server = setupServer(
  rest.get('https://front-assignment.codeit.team/courses.json', (req, res, ctx) => {
...
test('loads and displays courses', async () => {
  ...
  expect(screen.getByRole('button', { name: '모든 타입 ,' })).toBeInTheDocument();
...
describe('when specific course type is selected', () => {
  ...
  test('when Legacy type is selected, it should only show Legacy type courses', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('option', { name: 'Legacy' })).toBeInTheDocument());
    fireEvent.click(screen.getByRole('option', { name: 'Legacy' }));

    expect(screen.queryByText('파이썬 기초')).toBeInTheDocument();
    expect(screen.queryByText('파이썬으로 시작하는 알고리즘')).not.toBeInTheDocument();
  });
  ...
  test('when V1 type is selected, it should only show V1 type courses', async () => {
    ...
    expect(screen.queryByText('파이썬으로 시작하는 알고리즘')).toBeInTheDocument();
  });
});
...
describe('when search query is typed and submitted', () => {
  ...
  test("when '파이썬 기초 Python' is typed and submitted, no course should be displayed", async () => {
    const input = screen.getByPlaceholderText('강의 이름, 언어, 코스 소개로 검색하세요.');
  ...
  return (
    <SWRConfig
      value={{
        dedupingInterval: 0,
        provider: () => new Map(),
        fetcher: async (resource, init) => fetch(resource, init).then((res) => res.json()),
      }}
    >
